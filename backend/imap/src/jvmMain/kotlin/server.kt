...
class ImapServerWriter(val writer: AsyncWriterStream) : AsyncWriterStream by writer {
    suspend fun writeResponse(tag: Tag, reply: ImapResponse) {
        println("IMAP >>> $tag $reply")
        tag.serialize(this)
        reply.serialize(this)
        writeLineEnd()
    ...
    while (true) {
        when (state) {
            ...
            State.Authenticated -> {
                ...
                when (taggedCommand.command) {
                    is SelectCommand -> {
                        writer.writeResponse(Tag.Untagged, FlagsResponse(flags = FlagsResponse.SYSTEM_FLAGS))
