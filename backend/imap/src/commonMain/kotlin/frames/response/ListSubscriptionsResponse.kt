...
data class ListSubscriptionsResponse(val attributes: List<String>, val delimiter: String, val name: String) : ImapResponse {
    override suspend fun serialize(output: AsyncWriter) {
        output.writeStringUtf8("LSUB (${attributes.joinToString(" ")}) \"$delimiter\" $name")
    }
}