...
public class AsyncByteReadChannelReader(channel: ByteReadChannel) : AsyncReader, ByteReadChannel by channel {
    override val bufferPool: Pool<Buffer> = DefaultBufferPool
    override val openForRead: Boolean get() = !isClosedForRead

    override suspend fun discard(n: Int): Int {
        return discard(n.toLong()).toInt()
    }

    override suspend fun readBytes(n: Int, dest: Buffer): Int {
        return read { source, start, endExclusive ->
...
public fun ByteReadChannel.toAsyncByteReadChannelReader(): AsyncByteReadChannelReader {
